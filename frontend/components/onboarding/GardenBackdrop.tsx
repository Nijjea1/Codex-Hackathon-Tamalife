...
export function GardenBackdrop({ strongerShade = false, hideSky = false }: { strongerShade?: boolean; hideSky?: boolean }) {
  ...
  useEffect(() => {
    daylight.value = withTiming(isDay ? 1 : 0, { duration: 800, easing: Easing.inOut(Easing.quad), reduceMotion: ReduceMotion.Never });
  }, [daylight, isDay]);

  const gardenMotion = useAnimatedStyle(() => ({
    transform: [
      { scale: 1.055 + breeze.value * 0.025 },
      { translateX: -10 + breeze.value * 20 },
      { translateY: -4 + breeze.value * 8 },
      { rotate: `${-0.18 + breeze.value * 0.36}deg` },
    ],
  }));
