...
export type GardenTooltipPlacement = {
  horizontal: "left" | "center" | "right";
  vertical: "above" | "below";
};

type GardenTooltipPlacementOptions = {
  actor: GardenPoint;
  actorSize: number;
  sceneWidth: number;
  sceneHeight: number;
...
export function getGardenTooltipPlacement({
  actor,
  actorSize,
  sceneWidth,
  sceneHeight,
  ...
  padding,
  gap,
}: GardenTooltipPlacementOptions): GardenTooltipPlacement {
  const actorCenterX = actor.x + actorSize / 2;
  const halfTooltipWidth = tooltipWidth / 2;
  const horizontal =
    actorCenterX - halfTooltipWidth < padding
      ? "left"
      : actorCenterX + halfTooltipWidth > sceneWidth - padding
        ? "right"
        : "center";
  const spaceAbove = actor.y - padding;
  const spaceBelow = sceneHeight - actor.y - actorSize - padding;
  const vertical =
    spaceAbove >= tooltipHeight + gap || spaceAbove >= spaceBelow ? "above" : "below";
